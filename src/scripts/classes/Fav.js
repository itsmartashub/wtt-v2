		...
		article.innerHTML = `
			<figure class="item__figure">
				<img class="item__img" src=${
					arr_obj_imgs[favItem.id - 1]
				} alt="watch-brown-white-${favItem.title}" />
			</figure>
			...
			<svg xmlns="http://www.w3.org/2000/svg" data-id=${
				favItem.id
			} class="item__icon item__icon--add-to-bag" width="16.5" height="21.786" viewBox="0 0 16.5 21.786">
			...
			<svg xmlns="http://www.w3.org/2000/svg" data-id=${
				favItem.id
			} class="item__icon item__icon--remove-from-fav" width="17.499" height="20.783" viewBox="0 0 17.499 20.783">
	...
	favLogic() {
		...
		$favItems.addEventListener("click", (e) => {
			if (e.target.closest(".item__icon--remove-from-fav")) {
				...
				$deleteItemFromFavBtn.parentElement.classList.add(
					"item--deleted"
				)
				_AlertNotification.addNotification({
					text: `Watch has been removed from Favourites!`,
				...
				setTimeout(() => {
					...
					$deleteItemFromFavBtn.parentElement.classList.remove(
						"item--deleted"
					)
				}, 400)
				...
				let isAlreadyInBag = arr_bag.find(
					(item) => item.id == forBagItem.id
				)
				...
				if (!isAlreadyInBag) {
					...
					this.arrFav = this.arrFav.filter(
						(item) => item.id != itemID
					)
					Storage.saveFav(this.arrFav)
					...
					$addItemToBagBtn.parentElement.classList.add(
						"item--deleted"
					)
					...
					setTimeout(() => {
						...
						$addItemToBagBtn.parentElement.classList.remove(
							"item--deleted"
						)
					}, 400)
					...
					if (window.location.pathname == "/allwatches.html") {
						let cardAddToBagBtn = document.querySelector(
							".card .btn-addtobag"
						)
						if (cardAddToBagBtn.dataset.id == itemID) {
							cardAddToBagBtn
								.closest(".card")
								.classList.add("card--added")
					...
					alert(
						`${forBagItem.title} watch is already in Bag! You can only remove this item from the Favourites.`
					)
					return
				}
			}
