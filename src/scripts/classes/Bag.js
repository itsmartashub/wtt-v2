		...
		article.innerHTML = `
			<figure class="item__figure">
				<img class="item__img" src=${
					arr_obj_imgs[bagItem.id - 1]
				} alt="watch-brown-white-${bagItem.title}" />
			</figure>
			...
			<svg xmlns="http://www.w3.org/2000/svg" class="item__icon item__icon--delete" data-id=${
				bagItem.id
			} width="17.499" height="20.783" viewBox="0 0 17.499 20.783">
		...
		$bagItems.addEventListener("click", (e) => {
			...
			if (e.target.closest(".item__icon--delete")) {
				...
				setTimeout(() => {
					...
					$deleteItemBtn.parentElement.classList.remove(
						"item--deleted"
					)
				}, 400)
				...
				Storage.saveBag(this.arrBag)
				this.setBagValues(this.arrBag)
				$increaseAmountBtn.nextElementSibling.innerText =
					currItem.amount
				...
				if (currItem.amount > 0) {
					Storage.saveBag(this.arrBag)
					this.setBagValues(this.arrBag)
					$decreaseAmountBtn.previousElementSibling.innerText =
						currItem.amount
				} else {
					$decreaseAmountBtn.parentElement.parentElement.classList.add(
						"item--deleted"
					)
					_AlertNotification.addNotification({
						// text: `${currItem.title} has been removed from Bag!`,
					...
					setTimeout(() => {
						$bagItems.removeChild(
							$decreaseAmountBtn.parentElement.parentElement
						)
						this.deleteFromBag(itemID)
						$decreaseAmountBtn.parentElement.parentElement.classList.remove(
							"item--deleted"
						)
					}, 400)
				}
