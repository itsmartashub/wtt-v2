...
export default class Observer {
	scroll_observer(
		_selectorScrolltopShow,
		_selectorScrolltopHide,
		_selectorScrolldownHide
	) {
		const observer = new IntersectionObserver((entries, observer) => {
			entries.forEach((entry) => {
				...
				if (
					...
					entry.target.className == _selectorScrolltopShow
				) {
					this.showScrollTop()
					this.showScrollDown()
				} else if (
					entry.isIntersecting &&
					entry.target.className == _selectorScrolltopHide
				) {
					this.hideScrollTop()
				}
				...
				if (
					entry.isIntersecting &&
					entry.target.className == _selectorScrolldownHide
				) {
					this.hideScrollDown()
		...
		let showScrolltopTriggerEl = document.querySelectorAll(
			`.${_selectorScrolltopShow}`
		)
		let hideScrolltopTriggerEl = document.querySelectorAll(
			`.${_selectorScrolltopHide}`
		)
		let hideScrolldownTriggerEl = document.querySelectorAll(
			`.${_selectorScrolldownHide}`
		)
	...
	hideScrollTop() {
		if ($scrollTop.classList.contains("scrolltop--show"))
	...
	showScrollDown() {
		if ($scrollDown.classList.contains("scrolldown--hidden"))
			$scrollDown.classList.remove("scrolldown--hidden")
	}
	...
	showFiltermobile() {
		if ($filtermobile.classList.contains("filtermobile--hidden"))
			$filtermobile.classList.remove("filtermobile--hidden")
	}
